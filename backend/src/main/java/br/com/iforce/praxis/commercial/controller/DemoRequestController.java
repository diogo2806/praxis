...
public class DemoRequestController {
    ...
    public ResponseEntity<?> create(@RequestBody DemoRequestRequest request) {
        ...
        jdbcTemplate.update(
                """
                INSERT INTO demo_requests (name, email, company, role, hiring_volume, message, source)
                VALUES (?, ?, ?, ?, ?, ?, ?)
                """,
                trim(request.name()),
                normalizeEmail(request.email()),
                trim(request.company()),
                nullIfBlank(request.role()),
                nullIfBlank(request.hiringVolume()),
                nullIfBlank(request.message()),
                nullIfBlank(request.source())
        );

        return ResponseEntity.status(HttpStatus.CREATED).body(Map.of(
                "message", "Solicitação recebida. Nossa equipe entrará em contato para combinar a demonstração."
        ));
    }
    ...
    private boolean hasMaxLength(String value, int max) {
    ...
    public record DemoRequestRequest(
            String name,
            String email,
            String company,
            String role,
