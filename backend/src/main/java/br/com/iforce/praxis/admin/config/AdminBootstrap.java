...
/**
 * Garante a existência de um operador ADMIN inicial vinculado ao empresa técnico {@code PLATFORM}.
 *
 * <p>O ADMIN é o operador da plataforma ({@code roles = ADMIN}, {@code empresaId = PLATFORM}).
 ...
 * aquele e-mail no empresa PLATFORM. As credenciais ficam apenas em variáveis de ambiente.</p>
 */
@Component
public class AdminBootstrap implements ApplicationRunner {
    ...
    public void run(ApplicationArguments args) {
        ...
        if (empresaRepository.findById(PLATFORM_EMPRESA_ID).isEmpty()) {
            log.warn("Empresa técnico PLATFORM ausente; operador ADMIN inicial não foi criado.");
            return;
        }
        if (userRepository.findFirstByEmailAndEmpresaId(email, PLATFORM_EMPRESA_ID).isPresent()) {
            return;
        }

        UserEntity admin = new UserEntity();
        admin.setEmpresaId(PLATFORM_EMPRESA_ID);
        ...
        log.info("Operador ADMIN inicial criado no empresa PLATFORM (e-mail mascarado: {}).", mask(email));
    }
