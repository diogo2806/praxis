...
Status: atualizado em 2026-07-15 após auditoria da branch main no commit `2c3e118ac797bd52ba02553f53d6f9a298f7e296` e conclusão documental de `INT16`.
...
|---|---|---|---|
| `INT16` | Concluído | `docs/INTEGRACAO-GUPY-PROVEDOR.md` passou a descrever `result_candidate_page_url` com JWT assinado do tipo `candidate_result`, empresa e tentativa, TTL próprio configurável, consumo por `CandidateResultPageService`, exemplo contratual e fluxo coerentes com a implementação atual. | Nenhuma para o alinhamento documental; homologação externa com vaga, callback e webhook reais permanece não comprovada. |
| `INT14` | Concluído | `GupyTestResultMapper` gera uma credencial JWT exclusiva do tipo `candidate_result`, com empresa, tentativa e TTL configurável de resultado, e publica essa credencial em `result_candidate_page_url`; `CandidateResultPageService` aceita somente esse tipo de token, consulta a tentativa pelo par empresa/tentativa e mantém a resposta pública limitada. | Nenhuma para a credencial pública de resultado; homologação externa não é comprovada pelo código. |
| `INT15` | Concluído | A documentação do provedor Gupy reproduz o JSON real do catálogo sem `category` e `level` artificiais, registra que `ABANDONED` e `EXPIRED` são rejeitados, descreve a credencial pública de resultado atual e mantém explícita a ausência de homologação. | Nenhuma para consistência documental; homologação externa permanece pendente. |
...
| `docs/INTEGRACAO-GUPY-PROVEDOR.md` | contrato Gupy e URL pública de resultado | O resumo, a matriz de compatibilidade, o exemplo de `TestResult`, a descrição da página pública, o diagrama e o checklist correspondem ao JWT `candidate_result` efetivamente produzido; a ausência de homologação externa permanece explícita. |
