import{useMutation as K,useQuery as ae,useQueryClient as se}from"@tanstack/react-query";import{createFileRoute as Pe,Link as U}from"@tanstack/react-router";import{Archive as De,CheckCircle2 as Re,Edit3 as Be,Plus as Ne,RefreshCw as Ee,Save as Ie,Send as Ke,Trash2 as Le,Workflow as Fe,X as Me}from"lucide-react";import{useEffect as Qe,useMemo as Te,useState as w}from"react";import{AppShell as Oe}from"@/components/app-shell";import{EmptyState as Ce,SkeletonRows as Se,StateBanner as Ae}from"@/components/praxis-ui";import{AlertDialog as _e,AlertDialogAction as $e,AlertDialogCancel as Ve,AlertDialogContent as He,AlertDialogDescription as Ue,AlertDialogFooter as We,AlertDialogHeader as Xe,AlertDialogTitle as Ge}from"@/components/ui/alert-dialog";import{Button as N}from"@/components/ui/button";import{addAssessmentJourneyStep as Ye,archiveAssessmentJourney as Ze,createAssessmentJourney as ea,deleteAssessmentJourneyStep as aa,getAssessmentJourney as ta,listAssessmentJourneys as sa,listSimulations as ia,publishAssessmentJourney as na,updateAssessmentJourney as oa,updateAssessmentJourneyStep as ra}from"@/lib/api/praxis";import{cn as we}from"@/lib/utils";export const Route=Pe("/jornadas")({head:()=>({meta:[{title:"Jornadas de avalia\xE7\xE3o - Pr\xE1xis"},{name:"description",content:"Monte, ordene e publique sequ\xEAncias de avalia\xE7\xF5es."}]}),component:la}),te="principal";function la(){const e=se(),[s,x]=w(null),[j,k]=w(""),[q,D]=w(""),[T,$]=w(""),[V,W]=w(!0),[O,X]=w(te),[i,J]=w(null),L=ae({queryKey:["assessment-journeys"],queryFn:sa,retry:!1}),R=ae({queryKey:["assessment-journey",s],queryFn:()=>ta(s),enabled:!!s,retry:!1}),F=ae({queryKey:["simulations"],queryFn:ia,retry:!1}),I=L.data??[],C=R.data??null,H=Te(()=>(F.data??[]).filter(t=>t.status==="published"||t.livePublishedVersionNumber!=null),[F.data]),z=K({mutationFn:ea,onSuccess:async t=>{k(""),D(""),x(t.id),await e.invalidateQueries({queryKey:["assessment-journeys"]})}}),M=K({mutationFn:()=>Ye(s,{simulationId:T,sequenceKey:O.trim()||te,required:V}),onSuccess:async t=>{$(""),x(t.id),await Q(e,t.id)}}),P=K({mutationFn:na,onSuccess:async t=>{J(null),await Q(e,t.id)}}),A=K({mutationFn:Ze,onSuccess:async t=>{J(null),await Q(e,t.id)}}),B=K({mutationFn:t=>aa(s,t),onSuccess:async()=>{J(null),await Q(e,s)}}),E=K({mutationFn:({step:t,direction:ee})=>ra(s,t.id,{orderIndex:t.orderIndex+ee}),onSuccess:async()=>Q(e,s)}),_=z.error??M.error??P.error??A.error??B.error??E.error,G=i?.action==="publish"?P.isPending:i?.action==="archive"?A.isPending:i?.action==="remove-step"?B.isPending:!1,Y=i?.action==="publish"?P.error:i?.action==="archive"?A.error:i?.action==="remove-step"?B.error:null;function Z(){j.trim()&&z.mutate({name:j.trim(),description:q.trim()||null})}function a(){!s||!T||!O.trim()||M.mutate()}function S(){if(i){if(i.action==="publish"){P.mutate(i.journeyId);return}if(i.action==="archive"){A.mutate(i.journeyId);return}B.mutate(i.stepId)}}return<Oe>
      ...
      <Ca confirmation={i}pending={G}error={Y}onCancel={()=>J(null)}onConfirm={S}/>
    </Oe>}function Na({journey:e,simulations:s,selectedSimulationId:x,required:j,sequenceKey:k,onSimulationChange:q,onRequiredChange:D,onSequenceKeyChange:T,onAddStep:$,addPending:V,onPublish:W,publishPending:O,onArchive:X,archivePending:i,onRemoveStep:J,onMoveStep:L}){const R=se(),[F,I]=w(!1),[C,H]=w(e.name),[z,M]=w(e.description??"");Qe(()=>{I(!1),H(e.name),M(e.description??"")},[e.id,e.name,e.description]);const P=K({mutationFn:()=>oa(e.id,{name:C.trim(),description:z.trim()||null}),onSuccess:async a=>{I(!1),await Q(R,a.id)}}),A=e.sequences,B=A.reduce((a,S)=>a+S.steps.length,0),E=e.status!=="archived",_=e.status==="draft"&&B>0,G=k.trim()||te,Y=new Set(A.find(a=>a.sequenceKey===G)?.steps.map(a=>a.simulationId)??[]),Z=s.filter(a=>!Y.has(a.id));return<section className="rounded-xl border border-border bg-card">
      <header className="flex flex-col gap-3 border-b border-border p-5 lg:flex-row lg:items-start lg:justify-between">
        ...
        <div className="flex flex-wrap gap-2">
          ...
          <N variant="outline"size="sm"disabled={!E||i}onClick={X}>
    ...
    </section>}function Ca({confirmation:e,pending:s,error:x,onCancel:j,onConfirm:k}){if(!e)return null;const q=Sa(e);return<_e open onOpenChange={D=>{!D&&!s&&j()}}>
      <He>
        <Xe>
          <Ge>{q.title}</Ge>
          <Ue>{q.description}</Ue>
    ...
    </section>}function Je({status:e}){const s={draft:"Rascunho",published:"Publicada",archived:"Arquivada"},x=e==="published"?"border-success/30 bg-success/10 text-success":e==="archived"?"border-border bg-muted text-muted-foreground":"border-warning/30 bg-warning/10 text-warning-foreground";return<span className={we("rounded-full border px-2 py-0.5 text-[11px]",x)}>
      {s[e]}
    </span>}async function Q(e,s){await e.invalidateQueries({queryKey:["assessment-journeys"]}),s&&await e.invalidateQueries({queryKey:["assessment-journey",s]})}