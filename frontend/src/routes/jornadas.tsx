...
function AssessmentJourneysPage() {
  ...
  const publishMutation = useMutation({
    ...
    onSuccess: async (journey) => invalidateJourney(queryClient, journey.id),
  });
        ...
        <header className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
          <div>
            ...
            <p className="mt-2 max-w-3xl text-sm leading-6 text-muted-foreground">
              Organize avaliações publicadas em sequências. Convites, validade e acompanhamento ficam centralizados em Participações.
            </p>
          </div>
          <div className="flex flex-wrap gap-2">
            <Button asChild className="gap-2">
              <Link to="/participacoes/jornada"><Send className="h-4 w-4" />Criar convite</Link>
            </Button>
            <Button
              type="button"
              variant="outline"
        ...
        <div className="grid gap-5 lg:grid-cols-[300px_minmax(0,1fr)]">
          <aside className="space-y-5">
            ...
            <section className="overflow-hidden rounded-xl border border-border bg-card">
              <div className="border-b border-border p-4"><h2 className="text-sm font-semibold">Jornadas</h2></div>
                      ...
                      <div className="flex items-start justify-between gap-3">
                        <div className="min-w-0">
                          ...
                          <div className="mt-1 text-xs text-muted-foreground">
                            {journey.stepCount} avaliação(ões) · {journey.sequenceCount} sequência(s)
                          </div>
                        </div>
          ...
          <section className="space-y-5">
            {!selectedJourneyId ? (
              ...
              <div className="rounded-xl border border-border bg-card p-5"><SkeletonRows rows={6} /></div>
            ) : selectedJourneyQuery.isError || !selectedJourney ? (
              <StateBanner tone="danger" title="Não foi possível carregar a jornada">
                {selectedJourneyQuery.error instanceof Error ? selectedJourneyQuery.error.message : "Tente novamente."}
              </StateBanner>
            ) : (
              <>
                <JourneyComposer
...
function JourneyComposer({
  ...
  const simulationsInSequence = new Set(
    sequences.find((sequence) => sequence.sequenceKey === normalizedSequence)?.steps.map((step) => step.simulationId) ?? [],
  );
  const availableSimulations = simulations.filter((simulation) => !simulationsInSequence.has(simulation.id));
    ...
    <section className="rounded-xl border border-border bg-card">
      <header className="flex flex-col gap-3 border-b border-border p-5 lg:flex-row lg:items-start lg:justify-between">
        <div>
          ...
          <p className="mt-2 text-xs text-muted-foreground">{totalSteps} avaliação(ões) em {sequences.length} sequência(s)</p>
        </div>
        <div className="flex flex-wrap gap-2">
          {canEdit && (
            <Button variant="outline" size="sm" onClick={() => setEditing((current) => !current)}>
              <Edit3 className="mr-1.5 h-4 w-4" />{editing ? "Fechar edição" : "Editar"}
            </Button>
          )}
          <Button variant="outline" size="sm" disabled={!canPublish || publishPending} onClick={onPublish}>
            <CheckCircle2 className="mr-1.5 h-4 w-4" />Publicar
          </Button>
          <Button variant="outline" size="sm" disabled={!canEdit || archivePending} onClick={onArchive}>
      ...
      {editing && canEdit && (
        <div className="border-b border-border bg-muted/20 p-5">
          <div className="grid gap-3 lg:grid-cols-[1fr_1fr_auto]">
            ...
            <textarea className="input min-h-10" value={editDescription} onChange={(event) => setEditDescription(event.target.value)} />
            <div className="flex gap-2">
              <Button disabled={!editName.trim() || updateMutation.isPending} onClick={() => updateMutation.mutate()}><Save className="mr-1.5 h-4 w-4" />Salvar</Button>
              <Button variant="outline" disabled={updateMutation.isPending} onClick={() => setEditing(false)}><X className="mr-1.5 h-4 w-4" />Cancelar</Button>
            </div>
          </div>
        </div>
      )}
      ...
      {canEdit && (
        <div className="border-b border-border p-5">
          {simulations.length === 0 ? (
            <EmptyState title="Nenhuma avaliação publicada" description="Publique uma avaliação antes de adicioná-la à jornada." actions={<Button asChild><Link to="/avaliacoes">Abrir avaliações</Link></Button>} />
          ) : (
            <div className="grid gap-3 lg:grid-cols-[1fr_220px_150px_130px]">
              <select className="input" value={selectedSimulationId} onChange={(event) => onSimulationChange(event.target.value)}>
                <option value="">Selecione uma avaliação</option>
                {availableSimulations.map((simulation) => (
                  <option key={simulation.id} value={simulation.id}>{simulation.name} · v{simulation.livePublishedVersionNumber ?? simulation.versionNumber}</option>
                ))}
              </select>
              <input className="input" list="journey-sequences" value={sequenceKey} onChange={(event) => onSequenceKeyChange(event.target.value)} placeholder="Sequência" />
              <datalist id="journey-sequences">{sequences.map((sequence) => <option key={sequence.sequenceKey} value={sequence.sequenceKey} />)}</datalist>
              <label className="inline-flex items-center gap-2 rounded-md border border-border px-3 text-sm"><input type="checkbox" checked={required} onChange={(event) => onRequiredChange(event.target.checked)} />Obrigatória</label>
              <Button disabled={!selectedSimulationId || !sequenceKey.trim() || addPending} onClick={onAddStep}><Plus className="mr-1.5 h-4 w-4" />Adicionar</Button>
            </div>
          )}
        </div>
      )}
      ...
      {totalSteps === 0 ? (
        <div className="p-5 text-sm text-muted-foreground">Adicione pelo menos uma avaliação para publicar a jornada.</div>
      ) : (
        <div className="divide-y divide-border">
          {sequences.map((sequence) => (
            <div key={sequence.sequenceKey} className="p-5">
              <div className="mb-3 flex items-center gap-2">
                <span className="rounded-full border border-primary/30 bg-primary/10 px-2.5 py-0.5 text-xs font-medium text-primary">{sequence.sequenceKey}</span>
                <span className="text-xs text-muted-foreground">{sequence.steps.length} etapa(s)</span>
              </div>
              <div className="overflow-x-auto">
                <table className="w-full min-w-[680px] text-left text-sm">
                  <thead className="border-b border-border bg-muted/45 text-xs uppercase text-muted-foreground">
                    <tr><th className="px-4 py-3">Ordem</th><th className="px-4 py-3">Avaliação</th><th className="px-4 py-3">Versão</th><th className="px-4 py-3">Obrigatória</th><th className="px-4 py-3 text-right">Ações</th></tr>
                  </thead>
                  <tbody>
                        ...
                        <td className="px-4 py-3 text-right">
                          {canEdit && (
                            <div className="flex justify-end gap-2">
                              <Button variant="outline" size="sm" disabled={index === 0} onClick={() => onMoveStep(step, -1)}>Subir</Button>
                              <Button variant="outline" size="sm" disabled={index === sequence.steps.length - 1} onClick={() => onMoveStep(step, 1)}>Descer</Button>
                              <Button variant="outline" size="sm" className="text-danger" onClick={() => onRemoveStep(step.id)}><Trash2 className="h-3.5 w-3.5" /></Button>
                            </div>
                          )}
                        </td>
                      </tr>
  ...
  return (
    <section className="rounded-xl border border-border bg-card p-5">
      ...
      <p className="mt-1 text-sm text-muted-foreground">
        Esta jornada é configurada aqui. Pessoas participantes, links, validade, reenvios e resultados são administrados na Central de Participações.
      </p>
      <div className="mt-4 flex flex-wrap gap-2">
        <Button asChild disabled={!published}><Link to="/participacoes/jornada">Criar convite por jornada</Link></Button>
        <Button asChild variant="outline"><Link to="/participacoes">Abrir participações</Link></Button>
      </div>
      {!published && <p className="mt-3 text-xs text-muted-foreground">Publique a jornada para liberar a criação de convites.</p>}
    </section>
  );
}
...
function JourneyStatusBadge({ status }: { status: AssessmentJourneyStatus }) {
  ...
  return <span className={cn("rounded-full border px-2 py-0.5 text-[11px]", className)}>{label[status]}</span>;
}

async function invalidateJourney(queryClient: ReturnType<typeof useQueryClient>, journeyId: string | null) {
  await queryClient.invalidateQueries({ queryKey: ["assessment-journeys"] });
  if (journeyId) {
