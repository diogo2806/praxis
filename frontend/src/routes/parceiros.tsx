...
function PartnersWorkspace() {
  ...
  const specialistMutation = useMutation({
    mutationFn: () =>
      invitePartnerSpecialist({ name: specialistName.trim(), email: specialistEmail.trim() }),
    onSuccess: async () => {
      setSpecialistName("");
      setSpecialistEmail("");
      setActionError(null);
      ...
      <main className="mx-auto max-w-6xl space-y-8">
        ...
        <section className="space-y-4 rounded-xl border border-border bg-card p-5">
          <div>
            ...
            <p className="text-sm text-muted-foreground">
              Especialistas criam e revisam rascunhos, sem administrar clientes, cobrança ou
              publicação.
            </p>
          </div>
                    ...
                    <tr key={specialist.id}>
                      ...
                      <td className="px-4 py-3 text-right">
                        <Button
                          ...
                          disabled={removeSpecialistMutation.isPending}
                        >
                          Remover função
                        </Button>
                      </td>
        ...
        <section className="space-y-4 rounded-xl border border-border bg-card p-5">
          <div>
            ...
            <p className="text-sm text-muted-foreground">
              Cadastre o cliente, libere o catálogo e gere a credencial de integração.
            </p>
          </div>
          <div className="grid gap-3 md:grid-cols-[1fr_180px_1fr_auto] md:items-end">
            ...
            <div className="space-y-1.5">
              <Label htmlFor="client-external-id">ID externo</Label>
              <Input
            ...
            <Button
              ...
              disabled={clientMutation.isPending || !clientName.trim() || !externalCompanyId.trim()}
            >
              <Plus className="mr-2 h-4 w-4" />
              {clientMutation.isPending ? "Salvando..." : "Adicionar"}
            </Button>
          </div>
                    ...
                    <tr key={client.id}>
                      <td className="px-4 py-3">
                        ...
                        <div className="text-xs text-muted-foreground">
                          {client.externalCompanyId} · {client.active ? "Ativo" : "Inativo"}
                        </div>
                      </td>
                      <td className="px-4 py-3">{providerLabels[client.provider]}</td>
                      <td className="px-4 py-3">
                        {client.assignedTests} teste{client.assignedTests === 1 ? "" : "s"}
                      </td>
                      <td className="px-4 py-3">
                      ...
                      <td className="px-4 py-3 text-right">
                        <div className="flex flex-wrap justify-end gap-2">
                          ...
                          <Button
                            ...
                            disabled={!client.active || tokenMutation.isPending}
                          >
        ...
        {selectedClient && (
          <section className="space-y-4 rounded-xl border border-border bg-card p-5">
            <div>
              <h2 className="text-lg font-semibold">Catálogo de {selectedClient.name}</h2>
              <p className="text-sm text-muted-foreground">
                Somente avaliações publicadas e marcadas ficam disponíveis para este cliente.
              </p>
            </div>
            {catalogQuery.isLoading ? (
              <div className="h-24 animate-pulse rounded-lg bg-muted" />
            ) : catalogQuery.isError ? (
              <StateBanner tone="danger" title="Não foi possível carregar o catálogo">
                Tente novamente.
              </StateBanner>
            ) : (
              <div className="grid gap-3 md:grid-cols-2">
                {catalogQuery.data?.map((item) => (
                  <label
                    ...
                    className="flex cursor-pointer items-start gap-3 rounded-lg border border-border bg-background p-4"
                  >
                    <input
                      type="checkbox"
                      className="mt-1"
        ...
        {generatedToken && (
          <section className="rounded-xl border border-warning/40 bg-warning/5 p-5">
            <div className="flex items-start gap-3">
              <KeyRound className="mt-0.5 h-5 w-5 text-warning-foreground" />
              <div className="min-w-0 flex-1">
                <h2 className="font-semibold">Token gerado para {generatedToken.clientId}</h2>
                ...
                <div className="mt-4 flex flex-col gap-2 sm:flex-row">
                  ...
                  <Button variant="outline" onClick={() => void copyGeneratedToken()}>
                    {copied ? (
