    ...
    <AppShell>
      <div className="mx-auto max-w-7xl space-y-6">
        ...
        <OperationalChecklist
          ...
          generatedLinks={totals.invites}
          completedAttempts={totals.completed}
        ...
        <div className="grid gap-5 xl:grid-cols-[1fr_0.9fr]">
          ...
          <DeliveryQueuePanel
            ...
            processingReady={processReadyMutation.isPending}
            reprocessingId={reprocessMutation.variables ?? null}
            onProcessReady={() => processReadyMutation.mutate()}
        ...
        <div className="space-y-5">
                ...
                <tbody>
                  {filteredAttempts.map((attempt) => (
                    <tr key={attempt.attemptId} className="border-b border-border last:border-0">
                      <td className="px-4 py-3 font-medium">{revealed ? attempt.candidateName : maskName(attempt.candidateName)}</td>
                      <td className="px-4 py-3 text-muted-foreground">{attempt.simulationName} - v{attempt.versionNumber}</td>
                      <td className="px-4 py-3"><StatusBadge status={attempt.status} active={attempt.active} /></td>
                      <td className="px-4 py-3 text-muted-foreground">{attempt.currentTurn}/{attempt.estimatedTurns}</td>
                      <td className="px-4 py-3 font-mono text-xs text-muted-foreground">{formatElapsed(attempt.elapsedSeconds)}</td>
                      <td className="px-4 py-3 text-muted-foreground">{formatRelativeTime(attempt.lastSignalAt)}</td>
                      <td className="px-4 py-3 text-right"><AttemptAction attempt={attempt} /></td>
                    </tr>
                  ))}
                  {filteredAttempts.length === 0 && (
                    <tr>
                      <td colSpan={7} className="px-4 py-8 text-center text-sm text-muted-foreground">Nenhuma tentativa encontrada para os filtros atuais.</td>
                    </tr>
                  )}
                </tbody>
              </table>
          ...
          <section className="overflow-hidden rounded-xl border border-border bg-card">
            ...
            <div>
              ...
              {simulations.length === 0 && (<div className="px-4 py-8 text-sm text-muted-foreground">Nenhuma avaliação publicada encontrada.</div>)}
            </div>
          </section>
        </div>
      </div>
        ...
        {items.map((item) => (
          <div key={item.label} className="rounded-lg border border-border bg-background p-3">
            <div className="flex items-start gap-2">
              {item.done ? <CheckCircle2 className="mt-0.5 h-4 w-4 text-success" /> : <Clock3 className="mt-0.5 h-4 w-4 text-warning" />}
              <div className="min-w-0">
                <div className="text-sm font-medium">{item.label}</div>
                <div className="mt-1 text-xs text-muted-foreground">{item.detail}</div>
                <a href={item.href} className="mt-2 inline-block text-xs font-medium text-primary hover:underline">{item.action}</a>
              </div>
            </div>
          </div>
        ))}
  ...
  return (
    <section className="overflow-hidden rounded-xl border border-border bg-card">
      <div className="flex items-center justify-between border-b border-border px-4 py-4">
        <div>
          <h2 className="flex items-center gap-2 text-sm font-semibold"><Bell className="h-4 w-4 text-primary" />Notificações operacionais</h2>
          <p className="mt-1 text-xs text-muted-foreground">Alertas internos gerados pela operação, incluindo falhas definitivas de entrega.</p>
        </div>
      </div>
      {loading ? (
        <div className="px-4 py-8 text-sm text-muted-foreground">Carregando notificações...</div>
      ) : error ? (
        <div className="px-4 py-8 text-sm text-danger">Não foi possível carregar notificações.</div>
      ) : recentNotifications.length === 0 ? (
        <div className="px-4 py-8 text-sm text-muted-foreground">Nenhuma notificação operacional.</div>
      ) : (
          ...
          {recentNotifications.map((notification) => (
            <article key={notification.id} className="px-4 py-3">
              <div className="flex items-start justify-between gap-3">
                <div className="min-w-0">
                  <div className="flex items-center gap-2 text-sm font-medium">{!notification.readAt && <span className="h-2 w-2 rounded-full bg-primary" />}{notification.title}</div>
                  <p className="mt-1 line-clamp-2 text-xs leading-5 text-muted-foreground">{notification.message}</p>
                  {notification.candidateName && <p className="mt-1 text-xs text-muted-foreground">Candidato: {notification.candidateName}</p>}
                </div>
      ...
      <div className="border-b border-border px-4 py-3">
        <div className="flex flex-wrap gap-2">
          {filters.map(([id, label]) => (
            <button key={id} type="button" onClick={() => onFilterChange(id)} className={cn("rounded-md border px-2.5 py-1 text-xs transition", filter === id ? "border-primary bg-primary text-primary-foreground" : "border-border bg-background text-muted-foreground hover:bg-accent")}>{label}</button>
          ))}
        </div>
      </div>
      {loading ? <div className="px-4 py-8 text-sm text-muted-foreground">Carregando entregas...</div> : error ? <div className="px-4 py-8 text-sm text-danger">Não foi possível carregar entregas.</div> : deliveries.length === 0 ? <div className="px-4 py-8 text-sm text-muted-foreground">Nenhuma entrega para este filtro.</div> : (
            ...
            <tbody>
              {deliveries.map((delivery) => (
                <tr key={delivery.id} className="border-b border-border last:border-0">
                  <td className="px-4 py-3"><div className="font-medium">#{delivery.id}</div><div className="mt-0.5 text-xs text-muted-foreground">Attempt: {delivery.attemptId}</div><div className="mt-0.5 text-xs text-muted-foreground">Result: {delivery.resultId}</div></td>
...
function AttemptToggle({ value, onChange }: { value: AttemptFilter; onChange: (value: AttemptFilter) => void }) {
  const items: Array<[AttemptFilter, string]> = [["todos", "Todos"], ["ativos", "Ativos"], ["finalizadas", "Finalizadas"], ["sem-sinal", "Sem sinal"]];
  return <div className="inline-flex h-8 min-h-8 rounded-md border border-border bg-card p-0.5 text-xs">{items.map(([id, label]) => <button key={id} type="button" onClick={() => onChange(id)} className={cn("min-h-0 rounded px-2 py-0.5 transition", value === id ? "bg-primary text-primary-foreground" : "text-muted-foreground hover:bg-accent")}>{label}</button>)}</div>;
}

function StatusBadge({ status, active }: { status: AttemptStatus; active: boolean }) {
  const label = attemptStatusLabel(status, active);
  const tone = status === "completed" ? "border-success/30 bg-success/10 text-success" : active ? "border-primary/30 bg-primary/10 text-primary" : "border-warning/30 bg-warning/10 text-warning";
  return <span className={cn("inline-flex rounded-full border px-2 py-0.5 text-[11px]", tone)}>{label}</span>;
}

function DeliveryStatusBadge({ status }: { status: ResultDeliveryStatus }) {
  const config: Record<ResultDeliveryStatus, { label: string; className: string }> = {
    pending: { label: "Pendente", className: "border-warning/30 bg-warning/10 text-warning" },
    retrying: { label: "Retentativa", className: "border-primary/30 bg-primary/10 text-primary" },
    sent: { label: "Enviada", className: "border-success/30 bg-success/10 text-success" },
    dlq: { label: "DLQ", className: "border-danger/30 bg-danger/10 text-danger" },
  };
  return <span className={cn("inline-flex rounded-full border px-2 py-0.5 text-[11px]", config[status].className)}>{config[status].label}</span>;
}
...
function MetricCard({ icon, label, value, detail, tone }: { icon: ReactNode; label: string; value: number; detail?: string; tone: "ok" | "danger" }) {
  return <div className="rounded-xl border border-border bg-card p-4"><div className="flex items-center gap-2 text-[11px] font-medium uppercase text-muted-foreground">{icon}{label}</div><div className="mt-4 flex items-end gap-2"><div className="font-display text-3xl leading-none tabular-nums">{value}</div></div>{detail && <div className={cn("mt-2 text-xs", tone === "danger" ? "text-danger" : "text-muted-foreground")}>{detail}</div>}</div>;
}
...
function buildTotals(candidateLinks: CandidateLinkResponse[]) {
  ...
  return { invites, started, completed, adherence: Math.round((started / Math.max(invites, 1)) * 100), completion: Math.round((completed / Math.max(started, 1)) * 100) };
}
...
function formatElapsed(totalSeconds: number) {
  ...
  if (hours > 0) return `${String(hours).padStart(2, "0")}:${String(remainingMinutes).padStart(2, "0")}:${String(seconds).padStart(2, "0")}`;
...
function isReadyForRetry(delivery: ResultDeliveryResponse) {
  if (delivery.status !== "pending" && delivery.status !== "retrying") return false;
  if (!delivery.nextAttemptAt) return true;
  return new Date(delivery.nextAttemptAt).getTime() <= Date.now();
}
