...
import { AlertCircle, AlertTriangle, BadgeCheck, CalendarClock, CheckCircle2, CreditCard, Loader2, RefreshCw, XCircle } from "lucide-react";
...
function BillingPage() {
  ...
  return <AppShell><main className="space-y-4">
    ...
    {billing.isLoading ? <Loading /> : billing.isError || !billing.data ? <Failure retry={() => billing.refetch()} /> : <BillingView data={billing.data} plans={plans.data ?? []} management={management.data} plansLoading={plans.isLoading || management.isLoading} busy={busy} error={error} onCredit={(id) => credit.mutate(id)} onChange={(id) => change.mutate(id)} onSync={() => sync.mutate()} onCancel={() => cancel.mutate()} onEnterpriseRequest={(type, plan, note) => enterprise.mutate({ type, plan, note })} />}
  </main></AppShell>;
}

function BillingView({ data, plans, management, plansLoading, busy, error, onCredit, onChange, onSync, onCancel, onEnterpriseRequest }: {
  data: ClientBillingResponse; plans: ReturnType<typeof listClientBillingPlans> extends Promise<infer T> ? T : never; management: ReturnType<typeof getClientPlanManagement> extends Promise<infer T> ? T | undefined : never; plansLoading: boolean; busy: boolean; error: string | null; onCredit: (id: number) => void; onChange: (id: number) => void; onSync: () => void; onCancel: () => void; onEnterpriseRequest: (type: "CHANGE_PLAN" | "CANCEL_CONTRACT", plan?: CommercialPlanType, note?: string) => void;
}) {
...
function Usage({ data }: { data: ClientBillingResponse }) { return <Card title="Uso da plataforma" icon={<RefreshCw className="h-4 w-4" />}><div className="grid gap-3 sm:grid-cols-3"><Metric value={data.usage.completedLast7Days} label="Avaliações nos últimos 7 dias" /><Metric value={data.usage.completedLast30Days} label="Avaliações nos últimos 30 dias" /><Metric value={data.usage.completedAllTime} label="Avaliações concluídas no total" /></div></Card>; }
function Card({ title, icon, children }: { title: string; icon: React.ReactNode; children: React.ReactNode }) { return <section className="space-y-4 rounded-md border border-border bg-card p-6"><div className="flex items-center gap-2"><span className="text-muted-foreground">{icon}</span><h2 className="text-base font-semibold">{title}</h2></div>{children}</section>; }
