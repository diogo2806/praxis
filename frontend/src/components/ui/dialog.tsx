...
>(({ className, children, ...props }, ref) => {
  ...
  const hasViewportHeight = className?.includes("max-h-[88vh]") ?? false;

  React.useEffect(() => {
    setMounted(true);
  }, []);

  // Portais não participam do HTML inicial. Isso mantém o DOM de SSR e hidratação idênticos.
  if (!mounted) {
    return null;
  }
