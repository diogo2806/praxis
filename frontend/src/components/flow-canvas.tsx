...
type CompW = { name: string; weight: number };
...
export default function FlowCanvas({
  ...
  const perCompScore = (nodeId: string) => {
    ...
    return comps.map((c) => {
      ...
      return { name: c.name, acc: raw, pct: max > 0 ? Math.round((raw / max) * 100) : null };
    });
  };
  ...
  const organizeAutomatically = () => {
    const nextPositions = Object.fromEntries(
      nodes.map((node) => [node.id, autoLayout[node.id] ?? { x: 60, y: 60 }]),
    ) as Record<string, Pos>;

    setLocalPos(nextPositions);
    setPathFocusId(null);

    if (canEdit) {
      nodes.forEach((node) => {
        const p = nextPositions[node.id];
        onUpdateNode(node.id, { positionX: Math.round(p.x), positionY: Math.round(p.y) });
      });
    }
        ...
        {comps.map((c) => {
          ...
          return (
            <div className="vx-score-row" key={c.name} title={c.name}>
              <span className="vx-sr-name">{c.name}</span>
              <input
                type="number"
                min={0}
                max={100}
    ...
    <div className="vx-root">
      ...
      <div className="vx-bar">
        ...
        <div className="vx-tools">
          ...
          <button className="vx-ghost" onClick={organizeAutomatically} title="Reorganizar os cards pelo fluxo calculado"><ListTree size={15} /> Organizar automaticamente</button>
                      ...
                      <div className="vx-end-comps">
                        {perCompScore(n.id).map((c) => (
                          <div className="vx-end-comp" key={c.name} title={c.name}>
                            <span className="vx-ec-name">{c.name}</span>
        ...
        <aside className="vx-inspector">
          {!sel ? (
            ...
            <div className="vx-insp-body">
              ...
              <div className="vx-insp-end-comps">
                {perCompScore(sel.id).map((c) => (
                  <span key={c.name} title={c.name}><b>{c.name}</b> acum {c.acc}<em>{c.pct == null ? "—" : `${c.pct}%`}</em></span>
                ))}
              </div>
            ...
            <div className="vx-insp-body">
              ...
              {chainTo(sel.id).length > 1 && <div className="vx-insp-acc">acumulado até aqui:{comps.map((c) => { const a = accTo(sel.id); return <span key={c.name}>{c.name} <b>{a[c.name] ?? 0}</b></span>; })}</div>}
