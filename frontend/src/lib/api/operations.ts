...
import { PraxisApiError, type ResultDeliveryResponse, type ResultDeliveryStatus } from "@/lib/api/praxis";

export type InAppNotificationType = "RESULT_DELIVERY_DLQ" | string;
...
export function listNotifications() {
  return operationalRequest<InAppNotificationResponse[]>("/api/v1/notifications");
}

export function processReadyResultDeliveries() {
  return operationalRequest<ProcessReadyDeliveriesResponse>(
...
export function reprocessResultDelivery(deliveryId: number) {
  return operationalRequest<ReprocessDeliveryResponse>(
...
export function listDeliveriesByStatus(status: ResultDeliveryStatus) {
  return operationalRequest<ResultDeliveryResponse[]>(
    `/api/v1/gupy/result-deliveries?status=${encodeURIComponent(status)}`,
  );
}

async function operationalRequest<T>(path: string, init?: RequestInit): Promise<T> {
